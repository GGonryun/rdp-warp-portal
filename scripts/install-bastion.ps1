...
# ------------------------------------------------------------------
Write-Host "[1/14] Validating prerequisites..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[2/14] Creating directory structure..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[3/14] Installing RDS Session Host role..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[4/14] Installing RD Gateway role..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[5/14] Creating SSL certificate for RD Gateway..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[6/14] Configuring RD Gateway..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[7/14] Installing ffmpeg..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[8/14] Creating session user accounts..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[8b/14] Setting directory permissions for session users..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[9/14] Configuring RDS policies..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[9b/14] Locking down bastion desktop..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[10/14] Configuring firewall rules..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[11/14] Deploying session launch script..." -ForegroundColor Yellow

$launchScriptPath = "$InstallDir\scripts\session-launch.ps1"
if (-not (Test-Path $launchScriptPath)) {
    Write-Host "  ERROR: session-launch.ps1 not found at $launchScriptPath" -ForegroundColor Red
    Write-Host "  Run install.ps1 first to deploy the bundle files" -ForegroundColor Yellow
} else {
    Write-Host "  session-launch.ps1 already deployed ($launchScriptPath)" -ForegroundColor Green
}
...
# ------------------------------------------------------------------
Write-Host "[11b/14] Building PIN credential provider..." -ForegroundColor Yellow
...
if ($PSCmdlet.ShouldProcess($credProvDll, "Build and register PIN credential provider")) {
    ...
    if (Test-Path $credProvDll) {
        ...
        if ($isNative) {
            ...
            # a combined hash of the credprov source files against the stored hash.
            $srcFiles = Get-ChildItem "$InstallDir\credprov\src\*" -Include *.h,*.cpp | Sort-Object Name
            $allContent = ($srcFiles | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
            $currentHash = (Get-FileHash -InputStream ([IO.MemoryStream]::new([Text.Encoding]::UTF8.GetBytes($allContent))) -Algorithm SHA256).Hash
  ...
  if (-not $skipBuild) {
    ...
    if (-not $clExe) {
        ...
        Write-Host "  Installing C++ and .NET build tools (this may take several minutes)..." -ForegroundColor Cyan
        $installArgs = @(
    ...
    if (-not $clExe) {
        ...
        # ---- Verify C++ source files from bundle ----
        $srcDir = "$InstallDir\credprov\src"
        $defFile = "$InstallDir\credprov\PinCredentialProvider.def"
        $requiredFiles = @("$srcDir\common.h", "$srcDir\credential.h", "$srcDir\provider.h",
                           "$srcDir\dllmain.cpp", "$srcDir\provider.cpp", "$srcDir\credential.cpp",
                           $defFile)
        $missingFiles = $requiredFiles | Where-Object { -not (Test-Path $_) }
        if ($missingFiles) {
            Write-Host "  ERROR: Missing credprov source files:" -ForegroundColor Red
            $missingFiles | ForEach-Object { Write-Host "    $_" -ForegroundColor Red }
            Write-Host "  Run install.ps1 first to deploy the bundle files" -ForegroundColor Yellow
        } else {
            Write-Host "  Credprov source files found in $srcDir" -ForegroundColor Green
...
"@
            ...
            if ($buildExitCode -eq 0 -and (Test-Path $credProvDll)) {
                ...
                $currentHash | Out-File -Encoding ASCII $sourceHashFile
            } else {
...
'@
        ...
        } # end if source files found
    }
  } # end if (-not $skipBuild)
}

# ------------------------------------------------------------------
# Step 9: Build portal.exe (.NET RDP shell)
# ------------------------------------------------------------------
Write-Host "[12/14] Building portal.exe..." -ForegroundColor Yellow

$portalExe = "$InstallDir\bin\portal.exe"
$portalSrc = "$InstallDir\portal\src"

if (-not (Test-Path "$portalSrc\Portal.csproj")) {
    Write-Host "  ERROR: Portal source not found at $portalSrc" -ForegroundColor Red
    Write-Host "  Run gateway-agent.exe --install to extract the source files" -ForegroundColor Yellow
} else {
    # ---- Check if portal.exe is up to date via source hash ----
    $skipPortalBuild = $false
    $portalHashFile = "$InstallDir\portal\.source_hash"
    $srcFiles = Get-ChildItem "$portalSrc\*" -Include *.cs,*.csproj | Sort-Object Name
    $allContent = ($srcFiles | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
    $currentPortalHash = (Get-FileHash -InputStream ([IO.MemoryStream]::new([Text.Encoding]::UTF8.GetBytes($allContent))) -Algorithm SHA256).Hash

    if (Test-Path $portalExe) {
        $storedPortalHash = if (Test-Path $portalHashFile) { Get-Content $portalHashFile -ErrorAction SilentlyContinue } else { "" }
        if ($currentPortalHash -eq $storedPortalHash) {
            Write-Host "  portal.exe is up to date (source unchanged)" -ForegroundColor Green
            $skipPortalBuild = $true
        } else {
            Write-Host "  Source code changed since last build -- rebuilding" -ForegroundColor Yellow
        }
    }
    ...
    if (-not $skipPortalBuild) {
        if ($PSCmdlet.ShouldProcess($portalExe, "Build portal.exe from .NET source")) {
            ...
            if (-not $dotnetPath) {
                ...
                (New-Object Net.WebClient).DownloadFile(
                    "https://dot.net/v1/dotnet-install.ps1", $dotnetInstaller)
            ...
            Write-Host "  Building portal.exe with dotnet publish..." -ForegroundColor Cyan
            Push-Location $portalSrc
            try {
                & dotnet publish -c Release -r win-x64 --self-contained `
                    -p:PublishSingleFile=true `
                    -o "$InstallDir\bin" 2>&1 | ForEach-Object { Write-Host "    $_" }
                if ($LASTEXITCODE -eq 0 -and (Test-Path $portalExe)) {
                    Write-Host "  Built portal.exe successfully" -ForegroundColor Green
                    $currentPortalHash | Out-File -Encoding ASCII $portalHashFile
                } else {
                    Write-Host "  WARNING: portal.exe build failed (exit code: $LASTEXITCODE)" -ForegroundColor Yellow
                }
            } finally {
                Pop-Location
            }
...
# ------------------------------------------------------------------
# Step 10: Seed Example Credentials File
# ------------------------------------------------------------------
Write-Host "[13/14] Creating example credentials file..." -ForegroundColor Yellow
...
# ------------------------------------------------------------------
Write-Host "[14/14] Installing Gateway Agent service..." -ForegroundColor Yellow
...
if ($PSCmdlet.ShouldProcess($agentConfig, "Write agent configuration")) {
    $config = @{
        ...
        portal_exe              = "$InstallDir\bin\portal.exe"
...
if ($WhatIfPreference) {
    ...
    if ($needsReboot) {
        Write-Host "  A reboot is required to complete RDS installation." -ForegroundColor Yellow
        $answer = Read-Host "  Restart now? [Y/n]"
        if ($answer -eq '' -or $answer -match '^[Yy]') {
            Write-Host "  Restarting..." -ForegroundColor Gray
            Restart-Computer -Force
        } else {
            Write-Host "  Skipped. Run 'Restart-Computer' when ready." -ForegroundColor Yellow
        }
    } else {
    ...
    if ($needsTermServiceRestart) {
        ...
        $answer = Read-Host "  Restart TermService now? [Y/n]"
        if ($answer -eq '' -or $answer -match '^[Yy]') {
            ...
            Write-Host "  Skipped. Run 'Restart-Service -Name TermService -Force' when ready." -ForegroundColor Yellow
        }
    }
}
