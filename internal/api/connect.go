...
// launch portal.exe as the user's shell. Portal embeds the target RDP desktop
// via the ActiveX control, handles resize natively, and records the session.
// The user sees the target desktop filling their screen; the bastion desktop
// is never shown because portal.exe runs as the shell.
func (s *Server) handleRDPFile(w http.ResponseWriter, r *http.Request) {
	...
	rdpContent := fmt.Sprintf(
		"full address:s:%s:3389\r\n"+
			...
			"screen mode id:i:2\r\n"+
			"alternate shell:s:%s -ConfigPath \"%s\"\r\n"+
